      ...
      <div class="p-6 bg-white mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
        ...
        <div class="flex mt-3">
          <div class="flex gap-5 text-sm text-gray-500 w-full items-center justify-start">
            ...
            <div class="flex items-center"><i class="fas fa-heart mr-1"></i>${newsFeed[i].points}</div>
            <div class="flex items-center"><i class="far fa-clock mr-1"></i>${newsFeed[i].time_ago}</div>
          </div>
        </div>
      </div>    
    `);
  ...
  function makeComment(comments, called = 0) {
    ...
    for (let i = 0; i < comments.length; i++) {
      ...
      if (comments[i].comments.length > 0) {
        commentsList.push(makeComment(comments[i].comments, called + 1));
      }
    }
